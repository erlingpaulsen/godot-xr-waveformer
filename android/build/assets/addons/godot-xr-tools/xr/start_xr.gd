...
# Current refresh rate
var _current_render_target_size : Vector2
...
func initialize() -> bool:
	...
	if xr_interface:
		print("OpenXR: Interface found. Initialized already? ", str(xr_interface.is_initialized()))
...
func _setup_for_openxr() -> bool:
	...
	if "render_target_size_multiplier" in xr_interface:
		print("OpenXR: Setting render target size multiplier to ", str(render_target_size_multiplier))
	...
	if enable_passthrough and xr_interface.is_passthrough_supported():
		...
		print("OpenXR: Passthrough enabled")
		get_viewport().transparent_bg = true
		xr_interface.set_environment_blend_mode(XRInterface.EnvironmentBlendMode.XR_ENV_BLEND_MODE_ALPHA_BLEND)
...
func _on_openxr_session_begun() -> void:
	...
	# Get the reported refresh rate and render target size
	...
	_current_render_target_size = xr_interface.get_render_target_size()
	print("OpenXR: Render target size reported as ", str(int(_current_render_target_size.x)), "x", str(int(_current_render_target_size.y)))
...
func _set_enable_passthrough(p_new_value : bool) -> void:
	...
	if xr_interface:
		if enable_passthrough:
			...
			xr_interface.set_environment_blend_mode(XRInterface.EnvironmentBlendMode.XR_ENV_BLEND_MODE_OPAQUE)
